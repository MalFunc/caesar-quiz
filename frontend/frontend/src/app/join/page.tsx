...
import { joinGame, submitAnswer, type LeaderboardEntry, type WsMessage } from '@/utils/api';
...
export default function JoinPage() {
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const data = await joinGame(code, name);
      setGameId(data.game_id);
      setPlayerId(data.player_id);
      setStep('waiting');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Gagal join game');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAnswer = async (e: React.FormEvent) => {
    ...
    try {
      const data = await submitAnswer(gameId, {
        player_id: playerId,
        question_id: questionId,
        answer,
      });
      setFeedback(data.correct ? 'BENAR' : 'SALAH');
      answerInput.value = '';
    } catch (err) {
      setFeedback('ERROR');
            ...
            {feedback && (
              <div
                className={`
                  ...
                  text-2xl font-bold
                  px-4 py-2
                  ${feedback === 'BENAR' ? 'text-pixelGreen bg-black' : 'text-red-500 bg-black'}
                  border-2 border-black
                  rounded
                  drop-shadow
                `}
              >
                {feedback === 'BENAR' ? '✅ Jawaban Benar!' : feedback === 'SALAH' ? '❌ Jawaban Salah!' : error}
              </div>
            )}
